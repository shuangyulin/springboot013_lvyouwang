<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="线路分类" prop="xianlufenlei">
          <el-input v-model="ruleForm.xianlufenlei" 
              placeholder="线路分类" clearable  :readonly="ro.xianlufenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xianlufenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xianlufenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
